import { MultinexClient, ExecuteRequest } from "../src";
...
async function runSalesforceIntegration() {
  ...
  const safeRequest: ExecuteRequest = {
  ...
  if (safeResponse.auditLog.status === "allowed") {
    const sfResult = await salesforceAgent.performAction(
      safeRequest.context?.action || '', 
      safeRequest.context?.target
    );
  ...
  const unsafeRequest: ExecuteRequest = {
  ...
  if (unsafeResponse.auditLog.status === "allowed") {
    const sfResult = await salesforceAgent.performAction(
      unsafeRequest.context?.action || '', 
      unsafeRequest.context?.target
    );
    console.log("System Result:", sfResult);
  } else {
